...
### QC on the sequence data [bamstat]

QTLtools include a fast an eficient wat to 
...
knitr::kable(cis_nominals_FDR[1:30,], col.names = c(1:14),
             caption = 
             "List of to 30 cis eQTLs mapped using nominal pass and the FDR, the columns are,
    1. The phenotype ID,
    2. The chromosome ID of the phenotype,
    3. The start position of the phenotype,
    4. The end position of the phenotype,
...
vcf <- read.vcfR("/home/rstudio/Bed-Seq/Genotypes.vcf.gz", verbose = FALSE )
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
library(readr)
RPKM_all_bed_d <- read_table2("/home/rstudio/Bed-Seq/genes.50percent.chr22.bed.gz")
```
...
vcf <- read.vcfR("/home/rstudio/Bed-Seq/genotypes.chr22.vcf.gz", verbose = FALSE )
vcf<- data.frame(cbind(vcf@fix,vcf@gt))
rownames(vcf) <- vcf$ID
vcf_top10_cis_nomial <- vcf[cis_nominal_top10$ID,]
vcf_top10_cis_nomial <- vcf_top10_cis_nomial[!duplicated(vcf_top10_cis_nomial$ID),]
...
download.file("https://www.ebi.ac.uk/gwas/api/search/downloads/full", destfile ="GWAS_Hits.txt", method = "auto", quiet = FALSE, mode = "w",cacheOK = TRUE)

GWAS_Hits <- read_delim("GWAS_Hits.txt", 
    "\t", escape_double = FALSE, col_types = cols(CHR_POS = col_number()), 
    trim_ws = TRUE)
sub_GWAS_Hits <- subset(GWAS_Hits, GWAS_Hits$CHR_ID == 22 )


rtc_results <- rtc_results[order(rtc_results$RTC),] 
top_rtc_table <- subset(rtc_results, rtc_results$RTC > 0.9)


rtc_variants <- make.names(top_rtc_table$our_variant_start,unique = TRUE)
rownames(top_rtc_table) <- rtc_variants

GWAS_names <- make.names(sub_GWAS_Hits$CHR_POS, unique = TRUE)
rownames(sub_GWAS_Hits) <- GWAS_names

sub_GWAS_Hits[rownames(top_rtc_table),]
rownames(sub_GWAS_Hits)
rownames(top_rtc_table)


write.table(top_rtc_table, file = "GWAS3.txt",
            sep = " ")


aa <- sub_GWAS_Hits[!duplicated(sub_GWAS_Hits$CHR_POS), ]
rownames(aa) <- aa$CHR_POS

### Plot

color <- data.frame(rtc_results$RTC > 0.9)

par(mfrow=c(2,1))

plot(rtc_results$RTC,
     c(rtc_results$our_variant_start- rtc_results$other_variant_start),
     type = "h",
     ylab = "Distances from the GWAS hit",
     xlab = "RTC value")
points(rtc_results$RTC,c(rtc_results$our_variant_start- rtc_results$other_variant_start),
       col= "red",
       pch = 16,
       cex = 0.7)
abline(h=0)
...
col <- rbPal(10)[as.numeric(cut(rtc_results$RTC,breaks = 10))]


plot(rtc_results$our_variant_start,
     c(rtc_results$our_variant_start- rtc_results$other_variant_start),
     type = "h",
     ylab = "Distances from the GWAS hit",
     xlab = "Chromosome possition")
points(rtc_results$our_variant_start,
       c(rtc_results$our_variant_start- rtc_results$other_variant_start),
       col= col,
       pch = 16,
       cex = 1)
